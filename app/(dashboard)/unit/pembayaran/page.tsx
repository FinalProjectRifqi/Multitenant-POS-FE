...
export default function ProsesPembayaranPage() {
    ...
    return (
      <div className="p-8 flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Total {formatRupiah(total)} telah diterima.</p>
          <button
  ...
  return (
    <div className="p-8">
      <PageHeader title="Proses Pembayaran" description="Pilih metode pembayaran & konfirmasi transaksi" />
      ...
      <div className="max-w-lg space-y-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
          ...
          <div className="space-y-2 text-sm text-muted-foreground">
            <div className="flex justify-between"><span>Pizza Margherita</span><span>{formatRupiah(65000)}</span></div>
            <div className="flex justify-between"><span>Ayam Bakar</span><span>{formatRupiah(45000)}</span></div>
            <div className="flex justify-between"><span>Coca-Cola</span><span>{formatRupiah(15000)}</span></div>
            <div className="flex justify-between"><span>Jus Jeruk</span><span>{formatRupiah(7000)}</span></div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {PAYMENT_METHODS.map(({ id, label, icon: Icon }) => (
              ...
              >
                ...
                <span className={`text-xs font-semibold ${selected === id ? "text-primary" : "text-muted-foreground"}`}>
                  {label}
                </span>
              </button>
            ))}
        ...
        >
          <CreditCard className="w-4 h-4" /> Konfirmasi Pembayaran {formatRupiah(total)}
        </button>
      </div>
    </div>
  );
